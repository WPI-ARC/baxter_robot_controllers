...
{
    class BaxterRobotVelocityTorqueController
    {
    protected:
    ...
    public:
                                            ...
                                            const std::map<std::string, PIDParams> joint_controller_params) : nh_(nh)
        {
            ...
            // Figure out which arm we have
            const bool is_left = arc_helpers::CheckAllStringsForSubstring(joint_names_, "left");
            const bool is_right = arc_helpers::CheckAllStringsForSubstring(joint_names_, "right");
            ...
            const std::string root_name = "base";
            const std::string tip_name = (arm_ == LEFT_ARM) ? "left_gripper" : "right_gripper";
    ...
    //            const double safe_d_term = ClampValue(d_term, -std::abs(p_term), std::abs(p_term));
        ...
        inline void CommandTorques(const std::vector<double>& torques)
        {
            if (torques.size() == joint_names_.size())
            {
                ...
                command_msg.mode = baxter_core_msgs::JointCommand::TORQUE_MODE;
                command_msg.names = joint_names_;
        ...
        inline void VelocityTargetCallback(baxter_core_msgs::JointCommand velocity_target)
        {
            if (velocity_target.mode == baxter_core_msgs::JointCommand::VELOCITY_MODE)
            {
                if (velocity_target.names.size() == velocity_target.command.size())
                {
                    // Push the command into a map
                    std::map<std::string, double> command_map;
                    for (size_t idx = 0; idx < velocity_target.names.size(); idx++)
                    {
                        const std::string& name = velocity_target.names[idx];
                ...
                {
                    ROS_WARN("Invalid JointCommand: %zu names, %zu commands", velocity_target.names.size(), velocity_target.command.size());
                    target_velocities_valid_ = false;
                }
            }
            else
            {
                ROS_WARN("JointCommand mode is not VELOCITY_MODE");
                target_velocities_valid_ = false;
            }
        }
